...
- (void) layoutSubviews
{
    ...
    self.placeholderLabel.KN_top=self.textContainerInset.top; //设置UILabel 的 y值
    
    self.placeholderLabel.KN_left=self.textContainerInset.left+2;//设置 UILabel 的 x 值
    
    self.placeholderLabel.KN_width=self.KN_width-self.placeholderLabel.KN_left*2.0;
    CGSize maxSize =CGSizeMake(self.placeholderLabel.KN_width,MAXFLOAT);
    if ([self.myPlaceholder length]>0) {
        self.placeholderLabel.KN_height= [self.myPlaceholder boundingRectWithSize:maxSize options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : self.placeholderLabel.font} context:nil].size.height;
    }
...
- (void)textChangedNotification:(NSNotification *)notification {
    [super textChangedNotification:notification];
