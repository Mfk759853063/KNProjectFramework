...
@implementation UIImage (KNAddtions)

+ (UIImage *)KN_imageWithColor:(UIColor *)color {
    CGRect rect = CGRectMake(0.0f, 0.0f, 1.0f, 1.0f);
    return [self KN_imageWithColor: color rect: rect];
}
...
+ (UIImage *)KN_firtFrameImageFromVideoUrl:(NSURL *)videoUrl{
    ...
    CGImageRef firtFrameRef = [generate1 copyCGImageAtTime:time actualTime:NULL error:&err];
    return [[UIImage alloc] initWithCGImage:firtFrameRef];
}

- (UIImage *)KN_applyLightEffect
{
    UIColor *tintColor = [UIColor colorWithWhite:1.0 alpha:0.3];
    return [self KN_applyBlurWithRadius:30 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}

- (UIImage *)KN_applyExtraLightEffect
{
    UIColor *tintColor = [UIColor colorWithWhite:0.97 alpha:0.82];
    return [self KN_applyBlurWithRadius:20 tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)KN_applyTintEffectWithColor:(UIColor *)tintColor
{
    ...
    return [self KN_applyBlurWithRadius:10 tintColor:effectColor saturationDeltaFactor:-1.0 maskImage:nil];
}
...
+ (UIImage *)KN_compressImage:(UIImage *)image toMaxFileSize:(NSInteger)maxFileSize {
    ...
    if (imageData.length > maxFileSize) {
        ...
        CGFloat scale = 20;
        image = [image KN_imageByScalingAndCroppingForSize:CGSizeMake(image.size.width<image.size.height?image.size.width - scale * ratio:image.size.width-scale,image.size.height<image.size.width?image.size.height - scale * ratio:image.size.height-scale )];
        return [UIImage KN_compressImage:image toMaxFileSize:maxFileSize];
    }
